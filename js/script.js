...
const btnOrder = document.querySelector('.calc__order');
...
calcForm.addEventListener('submit', (event) => {
    ...
    if (calcForm.width.value > 0 && calcForm.length.value > 0) {
        const square = calcForm.width.value * calcForm.width.value;
        const price = square * tariff[calcForm.tariff.value];

        calcResultWrapper.style.display = 'block';
        btnOrder.classList.add('calc__order_show');
...
validator.onSuccess((event) => {
  ...
  .then((data) => {
    form.reset();
    alert(`Спасибо, мы с Вами обязательно свяжемся, ваша заявка под номером ${data.id}`);
  });
});
